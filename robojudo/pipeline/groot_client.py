...
class ModalityConfig:
    """Minimal stand-in for gr00t.data.types.ModalityConfig.

    This file is a modified copy of gr00t/policy/server_client.py with the
    gr00t package imports stripped, so RoboJuDo does not depend on the full
    GR00T dependency tree (torch, transformers) just to speak ZMQ.
    """
    ...
    def __repr__(self):
        return f"ModalityConfig({self.__dict__})"
...
class BasePolicy:
    """Minimal stand-in for gr00t.policy.BasePolicy (client side only).

    Mirrors the real get_action(): validate, delegate to _get_action, validate.
    PolicyClient supplies check_observation / check_action / _get_action.
    """
